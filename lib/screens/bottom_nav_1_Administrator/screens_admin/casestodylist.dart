import 'package:chart_harakia/screens/admin_services/castodygetlistservice.dart';

import 'package:chart_harakia/screens/bottom_nav_1_Administrator/screens_admin/castodyDetailslist.dart';
...
class _CustodyListScreenState extends State<CustodyListScreen> {
  ...
  Future<List<dynamic>?>? custodyFuture;
  List<dynamic> allCustodies = [];
  List<dynamic> filteredCustodies = [];
  ...
  void initState() {
    super.initState();
    custodyFuture = _fetchCustodies();
  }

  Future<List<dynamic>?> _fetchCustodies() async {
    String fullName = await _extractFullName();
    final custodyService = Castodygetlistservice(); // using same service class
    List<dynamic>? custodyList = await custodyService.Castodyfromservice();

    if (!mounted) return [];

    setState(() {
      _fullName = fullName;
      allCustodies = custodyList ?? [];
      filteredCustodies = allCustodies;
    });
  ...
  void _applyFilter(String query) {
    setState(() {
      filteredCustodies = allCustodies.where((custody) {
        final typeMatch =
            custody['custody_type']?.toLowerCase().contains(query.toLowerCase()) ?? false;

        final custodyDate = DateTime.tryParse(custody['delivery_date'] ?? '');
        final fromMatch = fromDate == null ||
            (custodyDate != null &&
                custodyDate.isAfter(fromDate!.subtract(const Duration(days: 1))));

        final toMatch = toDate == null ||
            (custodyDate != null &&
                custodyDate.isBefore(toDate!.add(const Duration(days: 1))));
  ...
  Widget _buildCustodyCard(dynamic custody) {
    String cardId = custody['name'].toString();
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => CastodyDetailsScreen(custodyItem: custody),
          ),
        );
              ...
              child: Column(
                ...
                children: [
                  _buildInfoRow(
                      ...
                      color: Colors.teal,
                      label: "نوع العهدة",
                      value: custody['custody_type']),
                  const SizedBox(height: 10), 
                  _buildInfoRow(
                      icon: Icons.date_range,
                      color: Colors.purpleAccent,
                      label: "تاريخ التسليم",
                      value: custody['delivery_date']),
                  const SizedBox(height: 10),
                  _buildInfoRow(
                      icon: Icons.tag,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("قائمة العهد", textDirection: TextDirection.rtl),
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildCustodyCard(filteredCustodies[index]),
                  );
                } else {
                  return const Center(child: Text('لا توجد بيانات عهد متاحة.'));
                }
              },
            ),
          ),
